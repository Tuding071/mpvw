...
internal class TouchGestures(private val observer: TouchGesturesObserver) {
    ...
    // --- Custom Center Gesture ---
    private var isCustomCenterTouch = false
    private var lastFrameSeekX = 0f
    private var frameSeekTriggerPx = 12f
    private var frameSeekEnabled = true
    private var frameSeekStepCount = 1
    ...
    fun syncSettings(prefs: SharedPreferences, resources: Resources) {
        ...
        // TODO: when we add settings integration, load these values:
        // frameSeekEnabled, frameSeekStepCount, frameSeekTriggerPx
    }
    ...
    fun onTouchEvent(e: MotionEvent): Boolean {
        ...
        when (e.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (isCustomCenterTouch) {
                    val diffX = point.x - lastFrameSeekX
                    if (frameSeekEnabled && abs(diffX) >= frameSeekTriggerPx) {
                        val direction = if (diffX > 0) 1f else -1f
                        sendPropertyChange(PropertyChange.FrameSeek, direction * frameSeekStepCount)
                        lastFrameSeekX = point.x
                    }
                    lastPos.set(point)
                    gestureHandled = true
                } else {
